...
class Neo4jUtil(object):
    ...
    # 插入关系
    def insert_relation(self, origin_label, dest_label, origin_node, dest_node, relation_name):

        # 查询该关系是否已经存在
        match_result = self.graph.run(
            "MATCH (a:{0})-[r:{4}]->(b:{1}) WHERE a.name = '{2}' AND b.name = '{3}' RETURN r".format(origin_label, dest_label, origin_node,
                                                                                                     dest_node, relation_name)).data()
    ...
    def create_column_relation(self, origin_name, dest_name, relation_name):
        ...
        # 查询该关系是否已经存在
        self.insert_relation(origin_label, dest_label, origin_node, dest_node, relation_name)